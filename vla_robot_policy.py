...
class VLARobotPolicy(nn.Module):
    def __init__(
        ...
        # 4. Action Heads (分離型)
        # アーム制御 (x,y,z, roll, pitch, yaw)
        self.arm_head = nn.Sequential(
            nn.Linear(hidden_dim, 256),
            nn.GELU(),
            nn.Linear(256, 6) # 6自由度
        )
    ...
    def forward(self, images, instructions, attention_mask=None):
            ...
            # 安全策: バッチサイズが一致しない場合（DataParallelのバグ対策）はスライスする
            ...
            # vla_robot_policy.py の forward の最後
        
        # アームはTanhで -1 ~ 1
            arm_action = torch.tanh(self.arm_head(features))
            
            # グリッパーは Tanh を外す！ (BCEWithLogitsLossのため)
